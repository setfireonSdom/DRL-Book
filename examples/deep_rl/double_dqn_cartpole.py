"""PyTorch Double DQN for CartPole with MPS-aware device selection."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PyTorch Double DQN on CartPole.")
...
def main() -> None:
    args = parse_args()
    _, episode_rewards, losses, average_test_reward, device = train_double_dqn_cartpole(
        DoubleDQNConfig(episodes=args.episodes)
    )
    ...
    plot_series(
        episode_rewards,
        title="Double DQN CartPole Reward",
        ylabel="Reward",
        running_window=20,
    )
    ...
    if losses:
        plot_series(
            losses,
            title="Double DQN CartPole Loss",
            ylabel="Loss",
